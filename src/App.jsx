...
import './App.css'
import { UserIcon, EyeIcon, LockIcon, TvIcon } from './Icons'
...
function PricingTable({ discount = 0, plans }) {
  const activePlans = plans || INITIAL_PLANS
  return (
        ...
        {activePlans.map(plan => (
          <article key={plan.id} className={`pricing-card pc-${plan.color}`}>
            ...
            <div className="pc-table">
              ...
              {plan.rows.map((r, i) => {
                // La lógica de applyDiscount y formatCLP se necesita aquí también.
                // Esto demuestra por qué deberían estar en un archivo `utils.js` e importarse en ambos lugares.
                const formatCLP = (num) => '$' + num.toLocaleString('es-CL');
                const applyDiscount = (base, pct) => {
